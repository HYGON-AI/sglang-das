...
__global__ void concat_mla_absorb_q_hip_kernel(
  ...
  const int64_t first_row =
      static_cast<int64_t>(blockIdx.x) * kRowsPerBlock +
      wave * kRowsPerWave;
...
#pragma unroll
  for (int row_in_wave = 0; row_in_wave < kRowsPerWave; ++row_in_wave) {
    ...
    reinterpret_cast<uint4*>(out_row)[lane] =
        reinterpret_cast<const uint4*>(a_row)[lane];

    if (lane < kBVecsPerRow) {
      const auto* b_row = b + idx_0 * b_stride_0 + idx_1 * b_stride_1;
      reinterpret_cast<uint4*>(out_row)[kAVecsPerRow + lane] =
          reinterpret_cast<const uint4*>(b_row)[lane];
    }
  }
}

void check_concat_tensor(
    const at::Tensor& tensor, const char* name, int64_t expected_last_dim) {
  ...
  TORCH_CHECK(
      tensor.size(2) == expected_last_dim,
      name,
      ".size(2) must be ",
      expected_last_dim);
  TORCH_CHECK(tensor.stride(2) == 1, name, " must be contiguous in its last dimension");
  TORCH_CHECK(
      tensor.stride(0) % kBf16PerVec == 0 &&
          tensor.stride(1) % kBf16PerVec == 0,
      name,
...
void concat_mla_absorb_q(at::Tensor a, at::Tensor b, at::Tensor out) {
  ...
  TORCH_CHECK(
      a.size(0) == b.size(0) && a.size(0) == out.size(0) &&
          a.size(1) == b.size(1) && a.size(1) == out.size(1),
  ...
  const int64_t num_blocks =
      (num_rows + kRowsPerBlock - 1) / kRowsPerBlock;
  TORCH_CHECK(
      num_blocks <= std::numeric_limits<uint32_t>::max(),
      "concat_mla_absorb_q input is too large");

  const at::cuda::OptionalCUDAGuard device_guard(device_of(a));
  const cudaStream_t stream = at::cuda::getCurrentCUDAStream();
  concat_mla_absorb_q_hip_kernel<<<
      static_cast<uint32_t>(num_blocks), kThreads, 0, stream>>>(
