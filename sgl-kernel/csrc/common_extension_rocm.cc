...
TORCH_LIBRARY_EXPAND(sgl_kernel, m) {
   ...
   */
  m.def(
      "dcu_create_flashmla_kv_indices(Tensor req_to_token, Tensor req_pool_indices,Tensor page_kernel_lens, Tensor? "
  ...
  m.def(
      "dcu_align_evict_mask_to_page_size(Tensor seq_lens,Tensor evict_mask,int page_size,int num_draft_tokens,int bs) "
      "-> ()");
  m.impl("dcu_align_evict_mask_to_page_size", torch::kCUDA, &dcu_align_evict_mask_to_page_size);
  m.def(
      "dcu_create_extend_after_decode_spec_info(Tensor verified_id, Tensor seq_lens, Tensor accept_lens, Tensor "
  ...
  m.def(
      "dcu_create_chunked_prefix_cache_kv_indices(Tensor req_to_token, Tensor req_pool_indices, Tensor chunk_starts, "
  ...
  m.def(
      "dcu_assign_extend_cache_locs(Tensor req_pool_indices, Tensor req_to_token, Tensor start_offset, Tensor "
  ...
  m.def(
      "dcu_assign_req_to_token_pool(Tensor req_pool_indices_ptr,Tensor req_to_token_ptr,Tensor "
  ...
  m.def(
      "dcu_alloc_extend_kernel(Tensor pre_lens_ptr, Tensor seq_lens_ptr, Tensor last_loc_ptr, Tensor free_page_ptr, "
      "Tensor out_indices, int bs, int page_size) -> ()");
  m.impl("dcu_alloc_extend_kernel", torch::kCUDA, &dcu_alloc_extend_kernel);
  m.def(
      "dcu_alloc_decode_kernel(Tensor seq_lens_ptr, Tensor last_loc_ptr, Tensor free_page_ptr, Tensor out_indices, int "
      "bs, int page_size) -> ()");
