...
@pytest.mark.parametrize("rope_scaling_type", ["default"], ids=["mrope_default"])
def test_mrope(
    model_name: str,
    ...
    config = root_config.get_text_config()
    ...
    if rope_theta is None:
        root_text_config = getattr(root_config, "text_config", {})
        if isinstance(root_text_config, dict):
            rope_theta = root_text_config.get("rope_theta")
        else:
            rope_theta = getattr(root_text_config, "rope_theta", None)
        if rope_theta is None:
            rope_theta = root_config.to_dict().get("text_config", {}).get("rope_theta")
    if rope_theta is None:
        rope_theta = 5000000
