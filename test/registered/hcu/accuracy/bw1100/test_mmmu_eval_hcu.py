...
register_hcu_ci(est_time=7200, suite="nightly-hcu-vlm", nightly=True)

DEFAULT_HCU_VLM_SERVER_ARGS = [
    "--attention-backend",
    "fa3",
...
DEFAULT_HCU_MMMU_MODEL_CANDIDATES = [
...
DEFAULT_HCU_MMMU_DATASET_CANDIDATES = [
    "/tmp/hcu_datasets/mmmu/mmmu/Math/test-00000-of-00001.parquet",
...
class HCULocalParquetMMMUEval(MMMUVLMEval):
    def __init__(
        self,
        dataset_path: str,
        num_examples: int,
...
def _get_model_env(name: str) -> str:
    ...
    if not model:
        for candidate in DEFAULT_HCU_MMMU_MODEL_CANDIDATES:
            if os.path.exists(candidate):
                return candidate
        raise AssertionError(
            "No local HCU MMMU model path found. Set "
            f"{name} to one of: {DEFAULT_HCU_MMMU_MODEL_CANDIDATES}"
        )
...
def _get_dataset_path_env(name: str) -> str:
    ...
    if not path:
        for candidate in DEFAULT_HCU_MMMU_DATASET_CANDIDATES:
            if os.path.exists(candidate):
                return _find_mmmu_parquet(candidate)
        raise AssertionError(
            "No local HCU MMMU parquet dataset found. Set "
...
def _get_server_args_env(name: str) -> list[str]:
    ...
    return list(DEFAULT_HCU_VLM_SERVER_ARGS)


class TestBW1100MMMUEvalHCU(unittest.TestCase):
    @classmethod
    def setUpClass(cls):
        cls.model = _get_model_env("SGLANG_HCU_MMMU_MODEL")
        cls.threshold = _get_float_env("SGLANG_HCU_MMMU_THRESHOLD", 0.35)
    ...
    def test_mmmu(self):
        ...
        try:
            process = popen_launch_server(
                ...
                other_args=_get_server_args_env("SGLANG_HCU_MMMU_SERVER_ARGS"),
            )
            ...
            eval_obj = HCULocalParquetMMMUEval(
