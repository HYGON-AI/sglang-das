...
def _get_model_env(name: str) -> str:
    ...
    if not model:
        return DEFAULT_HCU_MMMU_MODEL_CANDIDATES[0]
...
def _get_optional_path_env(name: str) -> str | None:
    ...
    if not path:
        if DEFAULT_HCU_MMMU_DATASET_PATH and os.path.exists(
            DEFAULT_HCU_MMMU_DATASET_PATH
        ):
            return DEFAULT_HCU_MMMU_DATASET_PATH
        return None
...
class TestBW1100MMMUEvalHCU(unittest.TestCase):
    ...
    def setUpClass(cls):
        ...
        cls.num_examples = _get_int_env("SGLANG_HCU_MMMU_NUM_EXAMPLES", 10)
        cls.num_threads = _get_int_env("SGLANG_HCU_MMMU_NUM_THREADS", 4)
        cls.max_tokens = _get_int_env("SGLANG_HCU_MMMU_MAX_TOKENS", 30)
        cls.dataset_path = _get_optional_path_env("SGLANG_HCU_MMMU_DATASET_PATH")
