...
class TestToolChoiceLlama32(CustomTestCase):
    ...
    def setUpClass(cls):
        ...
        cls.process = popen_launch_server(
            ...
            other_args=get_server_args(
                "SGLANG_HCU_TOOL_CHOICE_SERVER_ARGS",
                HCU_TEXT_SERVER_ARGS
                + ["--disable-cuda-graph", "--tool-call-parser", "llama3"],
            ),
        )
        cls.base_url += "/v1"
    ...
    @unittest.skip("Llama3.2 HCU smoke skips streaming JSON chunk assembly; model may emit list chunks for this prompt")
    ...
    @unittest.skip("Llama3.2 HCU smoke keeps stable tool_choice paths; complex nested parameter generation is model-behavior flaky")
...
@unittest.skip("HCU smoke keeps the local Llama3.2 parser path; skip broader model matrix")
