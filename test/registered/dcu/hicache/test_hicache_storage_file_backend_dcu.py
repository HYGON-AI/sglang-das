...
register_dcu_ci(
    ...
    disabled="DCU disabled retest: file-backend HiCache smoke was reduced to 1-GPU local small model, but still triggers ROCr VMFault and server connection refusal.",
)
...
class HiCacheStorageBaseMixin:
    ...
    def trigger_offloading_and_flush(self):
        ...
        # Trigger offloading
        self.send_request(self.gen_prompt(1), max_tokens=16)
    ...
    def test_basic_backup_and_prefetch(self):
        ...
        base_prompt = self.gen_prompt(64)

        # First request - populate cache
        print("Step 1: Populating cache with large context...")
        response1 = self.send_request(base_prompt, max_tokens=16)
        ...
        response2 = self.send_request(base_prompt, max_tokens=16)
        ...
        self.assertGreater(
            cached_tokens, 16, "Expected significant cached tokens for remote hit"
        )


@unittest.skip("DCU file-backend smoke keeps one short 1-GPU path only.")
...
class TestHiCacheStorageFileBackendDCU(HiCacheStorageBaseMixin, CustomTestCase):
...
@unittest.skip("DCU file-backend smoke skips long accuracy evaluation.")
