...
class SchedulerPPMixin:
    @DynamicGradMode()
    def _pp_preforward_expert_distribution_req(self: Scheduler, recv_reqs: List):
        """Relay an expert-distribution request downstream before handling it.

        The dump is a world-group collective, so a non-last stage that handled
        it first would block waiting for stages that have not seen it yet. The
        regular relay in the PP loop then skips this iteration's requests.
        """
        self._pp_reqs_preforwarded = False
        if self.pp_group.is_last_rank or not any(
        ...
        self._pp_reqs_preforwarded = True
            ...
            for mb_id in range(self.pp_loop_size):
                ...
                with torch.profiler.record_function("recv_requests"):
                    recv_reqs = self.ingest_requests(
                        before_process=self._pp_preforward_expert_distribution_req
                    )
                if (
                    not self.pp_group.is_last_rank
                    and not self._pp_reqs_preforwarded
                ):
                    self._pp_commit_comm_work(self.send_req_work)
