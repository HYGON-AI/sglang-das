...
class MLPSyncBatchInfo:
    ...
    def all_gather(self, device, group: torch.distributed.ProcessGroup):
        ...
        if self.scheduler_step_info is None:
            # Upstream six-field MLPSync path when PD Decode StepInfo sync is off.
...
class SchedulerDPAttnMixin:
    ...
    def prepare_mlp_sync_batch(self: Scheduler, local_batch: ScheduleBatch):
        # When SGLANG_ENABLE_PD_DECODE_STEPINFO_SYNC=1, attach decode queue /
        # epoch fields to MLPSync and use the dedicated scheduler Gloo group.
