    ...
    def free(self, free_index: torch.Tensor):
        ...
        if self.free_group is None:
            ...
            if free_page_indices.numel() == 0:
                return
            # Upstream replaced `release_pages` with a staged free list, so the
            # already-free test has to span both containers -- that is exactly
            # what get_all_free_pages() returns.
            free_page_indices = free_page_indices[
                ~torch.isin(free_page_indices, self.get_all_free_pages())
            ]
            if free_page_indices.numel() == 0:
                return
