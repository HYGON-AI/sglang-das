...
def _select_hf_quant_metadata(
    ...
    # Some multimodal checkpoints (kimi_k26) keep the legacy compressed-tensors
    # key under text_config as well.
    if text_config_mapping is not None:
        value = text_config_mapping.get("compression_config")
        if value is not None:
            return "text_config.compression_config", value
