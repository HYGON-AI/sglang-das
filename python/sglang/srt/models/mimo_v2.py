                ...
                if name in params_dict:
                    ...
                    if attn_tp_size == 1 and expected_fused_tp_size is not None:
                        ...
                        if is_swa and hasattr(self.config, "swa_num_attention_heads"):
                            total_q = self.config.swa_num_attention_heads * self.config.swa_head_dim
                            total_k = self.config.swa_num_key_value_heads * self.config.swa_head_dim
                            total_v = self.config.swa_num_key_value_heads * getattr(self.config, "swa_v_head_dim", self.config.swa_head_dim)
                        else:
                            total_q = self.config.num_attention_heads * self.config.head_dim
                            total_k = self.config.num_key_value_heads * self.config.head_dim
                            total_v = self.config.num_key_value_heads * getattr(self.config, "v_head_dim", self.config.head_dim)
                        ...
                        v_parts = [ch[per_rank_q + per_rank_k:per_rank_q + per_rank_k + per_rank_v] for ch in chunks]
