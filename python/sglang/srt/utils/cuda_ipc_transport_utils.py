...
class CudaIpcTensorTransportProxy:
    ...
    def _acknowledge_consumption(
        ...
        all_sync_meta = self.sync_data_meta
        if isinstance(all_sync_meta, dict):
            if sync_data_meta is not None:
                acknowledgements = [(sync_data_meta, 1)]
            else:
                acknowledgements = [(meta, 1) for meta in all_sync_meta.values()]
        elif all_sync_meta is not None:
            acknowledgements = [(sync_data_meta or all_sync_meta, consumer_count)]
        else:
            acknowledgements = []

        open(SHM_LOCK_FILE, "a").close()
        for meta, increment in acknowledgements:
            with open(SHM_LOCK_FILE, "w+") as f:
                fcntl.flock(f, fcntl.LOCK_EX)
                sync_flag = self.get_sync_flag_from_meta(meta)
                sync_flag += increment
    ...
    def acknowledge_consumption(self, consumer_count: int = 1):
        """Release IPC-pool slices when a cache hit needs no tensor copy."""
    ...
    def _copy_slice_tensor_to_target(
        ...
        with torch.cuda.device(rebuild_device):
            ...
            if isinstance(self.sync_data_meta, dict) and consumer_count > 1:
                self._acknowledge_consumption(consumer_count)
            else:
                self._acknowledge_consumption(
                    consumer_count,
                    sync_data_meta=sync_data_meta,
                )
