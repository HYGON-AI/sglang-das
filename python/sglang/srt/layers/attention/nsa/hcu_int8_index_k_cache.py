"""INT8 page-planar cache helpers for the HCU NSA indexer.

The packed ABI is intentionally shared with the scaled FP8 indexer cache:
each physical page has its 64 K vectors first, followed by 64 FP32 scales.
Unlike the FP8 cache, the K bytes are signed INT8 values.  The typed aliases
are constructed while the pool is allocated so the forward path does not need
to reinterpret or materialize the packed storage.
"""
...
def resolve_index_k_cache_mode(
    ...
    *,
    ...
    """Resolve the indexer-cache format and reject non-gfx936 INT8 opt-ins."""
    if int8_enabled is None:
        int8_enabled = envs.SGLANG_NSA_HCU_INT8_INDEX_K_CACHE.get()
    ...
    if int8_enabled:
        if not hcu_device or not gfx936_device:
            raise ValueError(
                "SGLANG_NSA_HCU_INT8_INDEX_K_CACHE=1 is only supported on HCU "
                "gfx936; gfx938 uses the native FP8 index-K cache."
            )
        if page_size != INDEX_K_PAGE_SIZE or index_head_dim != INDEX_K_HEAD_DIM:
            raise ValueError(
                "SGLANG_NSA_HCU_INT8_INDEX_K_CACHE=1 requires "
...
def index_k_workspace_bytes_per_token(mode: IndexKCacheMode) -> float:
    """Return the one-lane workspace and claim reservation for INT8 mode."""
...
def create_index_k_int8_aliases(
    ...
    """Create zero-copy INT8-K and FP32-scale aliases of page-planar storage."""
...
def _quantize_and_store_index_k_int8_reference(
    ...
    quantized = torch.clamp(
        torch.where(
            key_fp32 / scales[:, None] >= 0,
            torch.floor(key_fp32 / scales[:, None] + 0.5),
            torch.ceil(key_fp32 / scales[:, None] - 0.5),
        ),
        -127,
        127,
    ).to(torch.int8)
...
def quantize_and_store_index_k_int8(
    ...
    *,
    ...
    """Symmetrically quantize BF16 K and scatter it into page-planar storage."""
...
def dequantize_index_k_int8_paged(
    ...
    *,
    ...
    """Dequantize referenced physical pages once into a reusable BF16 workspace."""
    expected_page_bytes = INDEX_K_PAGE_SIZE * INDEX_K_BYTES_PER_TOKEN
    if (
        packed_cache.dtype != torch.uint8
        or packed_cache.ndim != 2
...
def validate_hcu_int8_index_k_cache_server_args(server_args) -> None:
    ...
    if unsupported:
        raise ValueError(
            "SGLANG_NSA_HCU_INT8_INDEX_K_CACHE=1 does not yet support "
            + ", ".join(unsupported)
            + "."
        )
