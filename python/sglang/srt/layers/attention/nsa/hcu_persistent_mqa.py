...
"""Runtime gate for the packaged gfx938 persistent-MQA fastpath."""
...
def resolve_persistent_ctas(
    ...
    """Resolve the packaged kernel's grid size without reading device lengths.

    A zero configuration selects the validated batch-aware formula. It depends
    only on capture-stable tensor shapes, so CUDA Graph replay may change the
    values in ``context_lens`` without changing the captured launch geometry.
    """
...
def _validate_configured_ctas(configured_ctas: int) -> None:
    if configured_ctas < 0 or configured_ctas > 4096:
        raise ValueError(
            "SGLANG_NSA_HCU_PERSISTENT_MQA_CTAS must be 0 (auto) or in "
...
def _consumer_gate_miss_reason(
    *,
    ...
    if not fuse_topk:
        return "SGLANG_NSA_FUSE_TOPK must be enabled"
...
def _select_package_operation(package_operation: Any) -> None:
    ...
    logger.info("Using packaged LightOp persistent-MQA operation")


def _raise_unavailable(reason: str) -> None:
    raise RuntimeError(f"HCU persistent-MQA fastpath unavailable: {reason}")
...
def _log_consumer_fallback(reason: str) -> None:
    if reason not in _logged_misses:
        ...
        logger.warning(
            "HCU persistent-MQA fastpath missed; using production LightOp: %s",
            reason,
        )
...
def preload_hcu_persistent_mqa(
    *,
    ...
    if not envs.SGLANG_NSA_HCU_PERSISTENT_MQA_FASTPATH.get():
        return
    ...
    try:
        _validate_configured_ctas(envs.SGLANG_NSA_HCU_PERSISTENT_MQA_CTAS.get())
...
def _log_hit_once(*, q_rows: int, max_context_len: int, resolved_ctas: int) -> None:
    ...
    logger.info(
        "HCU persistent-MQA fastpath HIT source=lightop-package q_rows=%d "
...
def paged_mqa_logits_length_masked(
    ...
    *,
    ...
    """Call the packaged length-masked kernel when every gate matches.

    Every ``context_lens`` value must already be in ``[0, max_context_len]``.
    ``clean_logits=True`` is a caller-side safety assertion retained by the
    packaged API; this operation guarantees only valid prefixes. The unchanged
    production TopK consumes ``context_lens`` and ignores the unwritten tail.
    """
    ...
    if reason is not None:
        # Consumer eligibility varies across captured forward modes. Treat a
        # miss as a normal production fallback, not a broken persistent-MQA
        # deployment. EAGLE's logical-index path is eligible: fast_topk_v2
        # consumes the same context_lens and ignores the unwritten score tail.
    ...
    try:
        resolved_ctas = resolve_persistent_ctas(
            envs.SGLANG_NSA_HCU_PERSISTENT_MQA_CTAS.get(),
            batch_size=q.shape[0],
            max_context_len=max_context_len,
        )
