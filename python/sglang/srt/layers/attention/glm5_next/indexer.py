...
if _is_hcu:
    ...
    from sglang.srt.layers.attention.glm5_next.triton_kernel import (
...
_HCU_MQA_LOGITS_ALIGNMENT = 128
_HCU_MQA_LOGITS_WORKSPACE_GB = envs.SGLANG_DSA_HCU_MQA_LOGITS_WORKSPACE_GB.get()
if _HCU_MQA_LOGITS_WORKSPACE_GB <= 0:
    raise ValueError("SGLANG_DSA_HCU_MQA_LOGITS_WORKSPACE_GB must be positive")
_HCU_MQA_LOGITS_MAX_ELEMENTS = int(_HCU_MQA_LOGITS_WORKSPACE_GB * (1 << 30)) // 4


def _is_hcu_mqa_logits_fp8_dtype(dtype: torch.dtype) -> bool:
    return dtype == torch.float8_e4m3fn


def _is_hcu_mqa_logits_compact_dtype(dtype: torch.dtype) -> bool:
    return _is_hcu_mqa_logits_fp8_dtype(dtype) or dtype == torch.int8


def reserve_hcu_mqa_logits_workspace(
    ...
    if device.type != "cuda":
        raise ValueError(
            f"HCU mqa_logits workspace requires a CUDA device, got {device}"
        )
    ...
    if dtype not in (
        ...
        raise ValueError(f"Unsupported HCU mqa_logits input dtype: {dtype}")
    return torch.empty(_HCU_MQA_LOGITS_MAX_ELEMENTS, dtype=torch.float32, device=device)


def _get_hcu_mqa_logits_output_shape(
    ...
    if dtype not in (
        ...
        raise ValueError(f"Unsupported HCU mqa_logits input dtype: {dtype}")
    if _is_hcu_mqa_logits_compact_dtype(dtype) or num_q < _HCU_MQA_LOGITS_ALIGNMENT:
        return num_q, num_k
    return (
        ceil_align(num_q, _HCU_MQA_LOGITS_ALIGNMENT),
        ceil_align(num_k, _HCU_MQA_LOGITS_ALIGNMENT),
    )


def _get_hcu_mqa_logits_max_rows(
    ...
    if _is_hcu_mqa_logits_compact_dtype(dtype):
        max_rows = workspace_numel // num_k
    elif dtype in (torch.float16, torch.bfloat16):
        aligned_k = ceil_align(num_k, _HCU_MQA_LOGITS_ALIGNMENT)
        max_aligned_rows = (
            workspace_numel // aligned_k // _HCU_MQA_LOGITS_ALIGNMENT
        ) * _HCU_MQA_LOGITS_ALIGNMENT
        if max_aligned_rows >= _HCU_MQA_LOGITS_ALIGNMENT:
            max_rows = max_aligned_rows
        else:
            # LightOp's M < 128 fallback writes a compact [M, N] tensor.
            max_rows = min(workspace_numel // num_k, _HCU_MQA_LOGITS_ALIGNMENT - 1)
    else:
        raise ValueError(f"Unsupported HCU mqa_logits input dtype: {dtype}")

    if max_rows < 1:
        raise RuntimeError(
            "HCU mqa_logits workspace cannot hold one logits row: "
            f"workspace_numel={workspace_numel}, num_k={num_k}, dtype={dtype}."
        )
    return min(num_q, max_rows)


def _run_hcu_mqa_logits_with_workspace(
    q: torch.Tensor,
    k: torch.Tensor,
    ...
    num_k = k.shape[0]
    output_rows, output_cols = _get_hcu_mqa_logits_output_shape(num_q, num_k, q.dtype)
    required_numel = output_rows * output_cols
    if required_numel > workspace.numel():
        raise RuntimeError(
            "HCU mqa_logits chunk exceeds its workspace: "
    ...
    if workspace.dtype != torch.float32 or workspace.device != q.device:
        raise ValueError(
            "HCU mqa_logits workspace must be FP32 on the same device as Q, got "
            f"workspace dtype={workspace.dtype}, device={workspace.device}, "
            f"q device={q.device}"
        )

    if _is_hcu_mqa_logits_compact_dtype(q.dtype):
        if kv_scale is None:
            raise ValueError(f"{q.dtype} HCU mqa_logits requires a KV scale tensor")
        kv_scale = kv_scale.to(torch.float32).flatten().contiguous()
    elif q.dtype in (torch.float16, torch.bfloat16):
        if kv_scale is not None:
            raise ValueError("FP16/BF16 HCU mqa_logits does not accept KV scale")
    else:
        raise ValueError(f"Unsupported HCU mqa_logits input dtype: {q.dtype}")
...
class Indexer(MultiPlatformOp):
    def __init__(
        ...
        # This source implementation owns its LightOp projection / rope path.
    ...
    def _get_topk_paged(
        ...
        if self._use_hcu_bf16_index_cache(forward_batch):
            ...
            if envs.SGLANG_DSA_HCU_USE_TRITON_PAGED_MQA.get():
    ...
    def _get_topk_ragged(
        ...
        if use_bf16_index_cache:
            ...
            kv_bf16 = torch.cat(
                [
                    get_token_to_kv_pool().get_index_k_continuous(
                        layer_id,
        ...
        if _is_hcu:
            if use_bf16_index_cache:
                ...
                hcu_k, hcu_k_scale = kv_fp8
                hcu_k_scale = hcu_k_scale.flatten().contiguous()

            # Always use the bounded external output on HCU. This deliberately
            ...
            hcu_logits_workspace = reserve_hcu_mqa_logits_workspace(device, q.dtype)
            max_rows = _get_hcu_mqa_logits_max_rows(
                hcu_logits_workspace.numel(), q_offset, k_offset, q.dtype
            )
            need_chunk = True
            free_mem = 0
        else:
        ...
        *,
        ...
        if _is_hcu:
            if self._use_hcu_bf16_index_cache(forward_batch):
                ...
                return
            buf = get_token_to_kv_pool().get_hcu_index_k_with_scale_write_buffer(
                layer_id=layer_id
            )
            ...
            op.fuse_act_quant_and_store_index_k_cache(
                ...
                get_token_to_kv_pool().page_size,  # page_size
                1e-5,  # eps
                False,  # use_ue8m0
                is_e4m3,  # is_e4m3
            )
            (
                get_token_to_kv_pool().commit_hcu_index_k_with_scale_write_buffer(
        ...
        if enable_dual_stream and forward_mode.is_decode_or_idle():
            ...
            if _is_hcu:
                if self._use_hcu_bf16_index_cache(forward_batch):
                    ...
                    k_buf = (
                        get_token_to_kv_pool().get_hcu_index_k_with_scale_write_buffer(
                            layer_id=layer_id
                        )
                    )
