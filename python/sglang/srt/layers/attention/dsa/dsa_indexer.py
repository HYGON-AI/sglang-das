...
class Indexer(MultiPlatformOp):
    ...
    def _get_topk_paged(
        ...
        if _is_hip and not _is_dcu and not _use_aiter_preshuffle:
    ...
    def _get_topk_ragged(
        ...
        assert device_index is not None, "q must be on an indexed CUDA device"
        ...
        if enable_dual_stream and forward_batch.forward_mode.is_decode_or_idle():
            ...
            if _is_dcu:
                ...
                if enable_dual_stream:
                    ...
                    # PCG splits on cache store and top-k, so defer the store to
                    # k_cache_and_topk_result after the gate projection.
            ...
            if self._use_dcu_bf16_index_cache(forward_batch):
                ...
                if weights_proj_lora:
                    raise RuntimeError(
                        "DSA indexer weights_proj LoRA is incompatible with TC piecewise CUDA graph; "
                        "remove the explicit prefill cuda-graph backend override or drop "
        ...
        if self.rotary_emb.is_neox_style:
            ...
            if self.alt_stream is not None:
                ...
                with torch.npu.stream(self.alt_stream):
                    ...
                    _wq_b_event = self.alt_stream.record_event()
