...
class DeepseekV4AttnBackend(
    ...
    def __init__(
        ...
        self._dsv4_bf16_flashmla_decode = (
            envs.SGLANG_DSV4_DCU_USE_BF16_FLASH_MLA.get()
        )
        ...
        if self._dsv4_bf16_flashmla_decode:
            if not _is_hcu:
                raise RuntimeError(
                    "SGLANG_DSV4_DCU_USE_BF16_FLASH_MLA is only supported on HCU"
                )
            if self.token_to_kv_pool.is_bf16_attention_kv_cache:
                logger.warning(
                    "SGLANG_DSV4_DCU_USE_BF16_FLASH_MLA is redundant because "
