...
def moe_reduce_dispatch(
    ...
    if 1 <= n <= 4:
        ...
        ops.moe_sum_opt1(
            inter_cache_view, out_hidden_states[begin_chunk_idx:end_chunk_idx]
        )
    else:
    ...
    if shared_output is not None:
        out_hidden_states[begin_chunk_idx:end_chunk_idx].mul_(
            routed_scaling_factor
        ).add_(shared_output[begin_chunk_idx:end_chunk_idx])
    else:
        out_hidden_states[begin_chunk_idx:end_chunk_idx].mul_(routed_scaling_factor)


def fused_experts_impl_int8_marlin(
    ...
    **_,
    ...
    for chunk in range((num_tokens // CHUNK_SIZE) + 1):
        ...
        if activation == "silu":
            if use_lightop:
                qintermediate_cache2, qa2_scale = fuse_silu_mul_quant(
                    intermediate_cache1.view(-1, N2)
                )
            else:
                torch.ops._C.silu_and_mul(
                    intermediate_cache2, intermediate_cache1.view(-1, N2)
                )
                qintermediate_cache2, qa2_scale = per_token_quant_int8(
                    intermediate_cache2
                )
        ...
        if use_lightop and shared_output is not None:
            op.moe_sum(
                input=intermediate_cache3.view(*intermediate_cache3.shape),
...
def fused_experts_impl_int8_marlin_minimax_m2(
    ...
    **_,
    ...
    intermediate_cache2 = torch.empty((M * top_k_num, N), device=adevice, dtype=adtype)
