...
__global__ void load_cache_to_device_buffer_kernel(
  ...
  for (int iter = 0; iter < ITERATIONS_PER_WARP_BUFFER; iter++) {
    ...
    if (warp_id == 0) {
#ifdef USE_ROCM
      // Wavefront64 is wider than NUM_WARPS for the block sizes used here.
      // Restrict each scan to this iteration's window so later offsets are not
      // overwritten by otherwise-active lanes.
...
#ifdef USE_ROCM
    // Keep the writeback loop within the launch bound used by ROCm builds.
