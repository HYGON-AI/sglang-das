...
#ifdef USE_ROCM
  // HIP has no elect.sync, and this header is included by every ROCm JIT
  // module; elect the lowest active lane instead.
  return get_lane_id() == static_cast<uint32_t>(__ffsll(__ballot(1)) - 1);
#else
  uint32_t pred;
  asm volatile(
      "{\n"
...
template <typename T>
SGL_DEVICE T shfl_down(mask_t active_mask, T value, int delta, int width = kWarpThreads) {
#ifdef USE_ROCM
  (void)active_mask;
...
template <typename T>
SGL_DEVICE T shfl_xor(mask_t active_mask, T value, int lane_mask, int width = kWarpThreads) {
#ifdef USE_ROCM
  (void)active_mask;
