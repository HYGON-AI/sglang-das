...
def fused_sigmoid_gating_delta_rule_update_kernel_hcu(
    A_log,
    a,
    dt_bias,
    softplus_beta,
    ...
    for _ in range(0, T):
        ...
        if HAS_EAGLE_TREE_CUSTOM_ATTN_MASK:
            ...
            if step_idx != 0 and cache_idx >= 0:
                ...
                if parent_step_idx != step_idx - 1:
                    step_offset = parent_step_idx * stride_h0_source
                    cache_ptr = (
                        intermediate_states_buffer
        ...
        if CACHE_INTERMEDIATE_STATES:
            if cache_idx >= 0:
                step_offset = step_idx * stride_h0_source
                cache_ptr = (
                    intermediate_states_buffer
                    + cache_idx * cache_steps * HV * K * V
                    + step_offset
...
def fused_sigmoid_gating_delta_rule_update(
    ...
    fused_sigmoid_gating_delta_rule_update_kernel_hcu[grid](
        A_log=A_log,
        a=a,
        dt_bias=dt_bias,
        ...
        cache_steps=(
            intermediate_states_buffer.stride(0) // (HV * K * V)
            if intermediate_states_buffer is not None
            else (cache_steps or 0)
        ),
