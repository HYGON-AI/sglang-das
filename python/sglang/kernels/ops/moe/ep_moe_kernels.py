...
def _fwd_kernel_ep_scatter_1_fused(
    ...
    for offset in range(0, numel, BLOCK):
        ...
        mask = offs < numel
        val = tl.load(
            topk_ids_ptr + offs, mask=mask, other=-1, eviction_policy="evict_last"
        )
...
def _fwd_kernel_ep_scatter_non_quant(
    ...
    for token_id_int32 in range(start_token_id, total_token_num, grid_num):
        ...
        for topk_idx in range(topk_num):
            expert_id = tl.load(recv_topk + token_id * recv_topk_stride0 + topk_idx)
            if expert_id >= 0:
...
def _build_m_indices_and_ep_scatter_not_use_groupgemm(
    ...
    expert_start_loc = torch.zeros_like(num_recv_tokens_per_expert_gpu)
    # This path already materialized `m_indices` through
    # `fused_build_m_indices_kernel` above and has no separate padded/valid
    # token split, so pass the same counts for both arguments: every slot is
    # valid, which reproduces the pre-`num_valid_tokens_per_expert` behavior.
...
def _per_token_quant_int8_kernel_opt(
    ...
    for token_idx in range(token_idx_start, num_total_tokens, grid_size):
        ...
        if is_valid_token:
            ...
            x = tl.load(x_ptr + token_idx * stride_x + cols, mask=mask, other=0.0).to(
                tl.float32
            )
            absmax = tl.maximum(tl.max(tl.abs(x)), 1e-10)
            scale_x = absmax / 127
...
def per_token_quant_int8_triton_opt(
    x: torch.Tensor, tokens_per_expert: Optional[torch.Tensor] = None
):
    if x.dim() != 3:
