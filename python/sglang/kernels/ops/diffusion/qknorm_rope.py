...
from sglang.srt.utils.common import is_dcu
...
def can_use_fused_inplace_qknorm_rope(
    ...
    # The current TVM-FFI implementation is CUDA source: its deferred JIT
    # includes cuda_bf16.h and uses __shfl_*_sync intrinsics.  DCU reports
    # through the CUDA-compatible platform interface, so capability probing
    # would otherwise return true and fail only on the first real forward.
    # Keep the model's numerically equivalent QK RMSNorm + RoPE path instead.
    if is_dcu():
        logger.info("Disabling CUDA-only fused QKNorm+RoPE JIT on DCU")
        return False
