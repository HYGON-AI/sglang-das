...
def test_output_contract_rejects_browser_incompatible_h264_profile(monkeypatch):
    payload = {
        "streams": [
            {
                "codec_type": "video",
    ...
    with pytest.raises(RuntimeError, match="browser-compatible H.264 High profile"):
        video_adapter._probe_minimax_h3_output_fields("/output/result.mp4")
