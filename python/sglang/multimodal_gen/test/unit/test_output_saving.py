...
def test_video_with_audio_uses_single_pass_encoder(tmp_path, monkeypatch):
    ...
    def mimsave_spy(path, frames, **kwargs):
        ...
        assert kwargs["output_params"] == ["-movflags", "+faststart"]
...
def test_video_audio_single_pass_failure_falls_back(tmp_path, monkeypatch):
    ...
    assert calls[0][2]["output_params"] == ["-movflags", "+faststart"]
    assert calls[1][2]["output_params"] == ["-movflags", "+faststart"]
...
def test_direct_cuda_ffmpeg_command_enables_mp4_faststart():
    command = output_utils._finalize_direct_cuda_video_command(
        ["ffmpeg", "-vcodec", "libx264", "-crf", "1"],
        "/output/h3.mp4",
    )
