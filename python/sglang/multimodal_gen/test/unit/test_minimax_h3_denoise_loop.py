...
from sglang.multimodal_gen.runtime.pipelines_core.stages.model_specific_stages.minimax_h3.stages.denoising import (
    MINIMAX_H3_CANCEL_FILE_EXTRA_KEY,
    minimax_h3_cancel_requested,
)


def test_request_scoped_cancel_token_is_observed_without_distributed_init(tmp_path):
    cancel_file = tmp_path / "request.cancel"
    batch = SimpleNamespace(
        extra={MINIMAX_H3_CANCEL_FILE_EXTRA_KEY: str(cancel_file)}
    )
    assert not minimax_h3_cancel_requested(batch, device=torch.device("cpu"))
    cancel_file.touch()
    assert minimax_h3_cancel_requested(batch, device=torch.device("cpu"))
