...
class MiniMaxH3Qwen3VLEncoder(TextEncoder):
    ...
    def __init__(self, config: MiniMaxH3Qwen3VLConfig) -> None:
        ...
        # The vendor 0.5.12 Qwen3-VL backbone predates encoder folding and has
        # no use_tensor_parallel constructor argument.  The current H3 service
        # runs TP=1 and has already demonstrated that one encoder replica fits
        # per rank, so use the vendor-compatible replicated construction.
        self.model = Qwen3VLModel(arch)
