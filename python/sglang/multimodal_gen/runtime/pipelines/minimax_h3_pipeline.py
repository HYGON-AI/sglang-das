...
class MiniMaxH3Pipeline(LoRAPipeline, ComposedPipelineBase):
    ...
    def _load_config(self):
        ...
        if model_variant is not None:
            ...
            # A local deployment may point --model-path directly at FL2VA or
            # Ref2VA. In that case the selected partition is already resolved.
            if (
                os.path.basename(os.path.normpath(self.model_path)).lower()
                != semantic_subfolder.lower()
            ):
