...
def _finalize_direct_cuda_video_command(
    command: list[str], save_file_path: str
) -> list[str]:
    """Add the browser-delivery options to the direct CUDA ffmpeg command."""

    return [
        *command,
        *_BROWSER_COMPATIBLE_MP4_OUTPUT_PARAMS,
        "-v",
        "warning",
        save_file_path,
    ]
    ...
    *,
    ...
    try:
        ...
        with _acquire_cuda_video_buffer(shape) as buffer:
            ...
            command = _finalize_direct_cuda_video_command(command, save_file_path)
