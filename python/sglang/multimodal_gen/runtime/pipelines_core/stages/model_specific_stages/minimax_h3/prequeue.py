...
from sglang.multimodal_gen.runtime.pipelines_core.stages.model_specific_stages.minimax_h3.constants import (
    ...
    MINIMAX_H3_REF_MAX_CLIP_DURATION_SECONDS,
    MINIMAX_H3_REF_MAX_TOTAL_AUDIO_DURATION_SECONDS,
    MINIMAX_H3_REF_MAX_TOTAL_VIDEO_DURATION_SECONDS,
    MINIMAX_H3_REF_MIN_CLIP_DURATION_SECONDS,
...
_REFERENCE_DURATION_EPSILON_SECONDS = 1e-3
...
def _positive_probe_duration(
    facts: dict[str, Any], *, key: str, condition_index: int
) -> float:
    try:
        duration = float(facts[key])
    except (KeyError, TypeError, ValueError) as exc:
        raise ValueError(f"conditions[{condition_index}] has no usable {key}") from exc
    if not math.isfinite(duration) or duration <= 0:
        raise ValueError(f"conditions[{condition_index}] has no usable {key}")
    return duration


def _validate_ref2va_reference_durations(
    plan: MiniMaxH3ResolvedPlan,
    probe_facts: dict[int, dict[str, Any]],
) -> None:
    """Enforce released per-clip and per-modality Ref2VA duration limits."""
    ...
    for material in plan.materials:
        condition_index = int(material.condition_index)
        facts = probe_facts[condition_index]
        if material.condition_type in {"video", "video_audio"}:
            duration = _positive_probe_duration(
                facts,
        ...
        elif material.condition_type == "audio":
            duration = _positive_probe_duration(
                facts,
        ...
        if (
            duration
            < MINIMAX_H3_REF_MIN_CLIP_DURATION_SECONDS
            - _REFERENCE_DURATION_EPSILON_SECONDS
            or duration
            > MINIMAX_H3_REF_MAX_CLIP_DURATION_SECONDS
            + _REFERENCE_DURATION_EPSILON_SECONDS
        ):
            raise ValueError(
                f"conditions[{condition_index}] {label} duration must be in "
                f"[{MINIMAX_H3_REF_MIN_CLIP_DURATION_SECONDS:g}, "
                f"{MINIMAX_H3_REF_MAX_CLIP_DURATION_SECONDS:g}] seconds, "
                f"got {duration:g}"
            )
    ...
    if (
        ...
        raise ValueError(
            "Ref2VA reference video total duration must not exceed "
            f"{MINIMAX_H3_REF_MAX_TOTAL_VIDEO_DURATION_SECONDS:g} seconds, "
            f"got {video_total:g}"
        )
    if (
        audio_total
        ...
        raise ValueError(
            "Ref2VA standalone audio total duration must not exceed "
            f"{MINIMAX_H3_REF_MAX_TOTAL_AUDIO_DURATION_SECONDS:g} seconds, "
            f"got {audio_total:g}"
        )
...
def minimax_h3_prepare_for_queue(batch: Any) -> MiniMaxH3ResolvedPlan:
    ...
    try:
        ...
        _validate_ref2va_reference_durations(plan, probe_facts)
