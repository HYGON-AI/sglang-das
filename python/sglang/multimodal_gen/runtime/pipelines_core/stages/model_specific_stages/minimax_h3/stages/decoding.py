    ...
    def _decode_audio(
        ...
        with self.use_declared_component(
            ...
            with torch.autocast(
                ...
                get_vae_decode_fn = getattr(self, "_get_vae_decode_fn", None)
                audio_decode = (
                    get_vae_decode_fn(
                        audio_vae,
                        server_args,
                        decode_fn=audio_vae.decode,
                        compiled_callable=getattr(
                            self, "_compiled_audio_vae_decode", None
                        ),
                    )
                    if callable(get_vae_decode_fn)
                    else audio_vae.decode
                )
    ...
    def forward(self, batch: Req, server_args: ServerArgs) -> OutputBatch:
        ...
        with self.use_declared_component(
            ...
            if visual_autocast_enabled:
                prepare_autocast_weights = getattr(
                    selected_video_vae, "prepare_decoder_autocast_weights", None
                )
                if callable(prepare_autocast_weights):
                    prepare_autocast_weights(video_vae_dtype)
            with torch.autocast(
                ...
                video_decode = (
                    ...
                    if callable(get_vae_decode_fn)
                    else selected_video_vae.decode_base
                )
