...
MINIMAX_H3_CANCEL_FILE_EXTRA_KEY = "minimax_h3_cancel_file"


class MiniMaxH3CancellationRequested(RuntimeError):
    """Raised synchronously on every rank at a denoising step boundary."""


def minimax_h3_cancel_requested(batch: Req, *, device: torch.device) -> bool:
    """Collectively observe the API process' request-scoped cancel token."""

    cancel_file = batch.extra.get(MINIMAX_H3_CANCEL_FILE_EXTRA_KEY)
    local_requested = isinstance(cancel_file, str) and bool(cancel_file) and os.path.isfile(
        cancel_file
    )
...
def _precompute_refined_prompt_embeds(
    ...
    *,
    ...
    if isinstance(refiner_params, dict):
        ...
        refiner_cu = refiner_params.cu_seqlens_q
    # This helper runs outside the root DiT forward. FSDP2 normally unshards
    # root-owned parameters from its forward pre-hook, so explicitly bracket
    # the direct helper call and restore the sharded state afterwards.
    unshard = getattr(model, "unshard", None)
    reshard = getattr(model, "reshard", None)
    did_unshard = callable(unshard) and callable(reshard)
    if did_unshard:
        # PyTorch FSDP2 otherwise treats the first nested refiner block call as
        # the root forward. Initialize from the actual DiT root before invoking
        # any request-static helper outside ``model.forward``.
        get_fsdp_state = getattr(model, "_get_fsdp_state", None)
        if callable(get_fsdp_state):
            get_fsdp_state()._lazy_init()
        unshard()
    try:
...
class MiniMaxH3DenoisingStage(DenoisingStage):
    ...
    def _offload_for_torch_compile_warmup(self, batch: Req):
        """Vendor-base compatibility for the newer compile warmup hook.

        This deployment runs with torch.compile disabled, so the newer base
        implementation is intentionally a no-op here.
        """
        del batch
        yield
    ...
    def _cache_dit_requested(self) -> bool:
        if getattr(self, "_minimax_h3_quality_profile", "lossless") != "lossless":
            return True
        base_cache_requested = getattr(super(), "_cache_dit_requested", None)
        return bool(base_cache_requested()) if callable(base_cache_requested) else False
    ...
    def _maybe_enable_cache_dit(
        ...
        base_cache_requested = getattr(super(), "_cache_dit_requested", None)
        generic_requested = (
            bool(base_cache_requested())
            if callable(base_cache_requested) and "quality" not in explicit_fields
            else False
        )
        desired_mode = (
            quality
    ...
    def _run_full_loop(self, batch: Req, server_args: ServerArgs) -> None:
        ...
        try:
            ...
            with (
                maybe_nvtx_range(
                    "denoising_loop", getattr(self, "current_use_nvtx", False)
                ),
                ...
                video_rows, audio_rows = minimax_h3_denoise_loop(
                    ...
                    model_forward=partial(
                        self._forward_dit, batch=batch, server_args=server_args
                    ),
                    positive=positive,
    ...
    def _profile_denoising_step(self, step_index: int, *, batch: Req):
        if minimax_h3_cancel_requested(
            batch,
            device=torch.device("cuda", torch.cuda.current_device()),
        ):
            raise MiniMaxH3CancellationRequested(
                f"MiniMax-H3 request {batch.request_id} was cancelled"
            )
        with (
            maybe_nvtx_range(
                f"denoising_step_{step_index}",
                getattr(self, "current_use_nvtx", False),
            ),
            StageProfiler(
                f"denoising_step_{step_index}",
                logger=logger,
        ...
        *,
        ...
        with set_forward_context(
            ...
            with torch.autocast(
                ...
                enabled=not server_args.disable_autocast,
            ):
                get_bcg_runner = getattr(self, "_maybe_get_bcg_runner", None)
                runner = get_bcg_runner(model) if get_bcg_runner is not None else None
                if runner is None:
