...
from sglang.multimodal_gen.runtime.pipelines_core.stages.model_specific_stages.minimax_h3.constants import (
    ...
    MINIMAX_H3_REF_MAX_AUDIO_FILES,
    MINIMAX_H3_REF_MAX_IMAGE_FILES,
    MINIMAX_H3_REF_MAX_VIDEO_FILES,
...
def _validate_ref2va_conditions(conditions: Sequence[Mapping[str, Any]]) -> None:
    """Enforce the released Ref2VA per-modality file-count contract."""

    image_count = sum(condition["type"] == "image" for condition in conditions)
    video_count = sum(
        condition["type"] in {"video", "video_audio"} for condition in conditions
    )
    audio_count = sum(condition["type"] == "audio" for condition in conditions)

    if image_count > MINIMAX_H3_REF_MAX_IMAGE_FILES:
        raise ValueError(
            "conditions for task 'ref2va' allow at most "
            f"{MINIMAX_H3_REF_MAX_IMAGE_FILES} image files, got {image_count}"
        )
    if video_count > MINIMAX_H3_REF_MAX_VIDEO_FILES:
        raise ValueError(
            "conditions for task 'ref2va' allow at most "
            f"{MINIMAX_H3_REF_MAX_VIDEO_FILES} video files, got {video_count}"
        )
    if audio_count > MINIMAX_H3_REF_MAX_AUDIO_FILES:
        raise ValueError(
            "conditions for task 'ref2va' allow at most "
            f"{MINIMAX_H3_REF_MAX_AUDIO_FILES} standalone audio files, "
            f"got {audio_count}"
        )
    if image_count + video_count == 0:
        raise ValueError(
            "conditions for task 'ref2va' cannot contain audio as the sole "
            "input; at least one image or video file is required"
        )
...
def minimax_h3_validate_canonical_request(
    ...
    **_extra_kwargs: Any,
    ...
    elif profile.task == MINIMAX_H3_TASK_REF2VA:
        _validate_ref2va_conditions(normalized_conditions)
