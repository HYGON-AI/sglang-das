...
def _probe_minimax_h3_output_fields(
    ...
    *,
    ...
    try:
        probe = subprocess.run(
            [
                ...
                "error",
                "-show_entries",
                "stream=codec_type,codec_name,profile,pix_fmt,width,height,avg_frame_rate,nb_frames,duration,sample_rate,channels:format=format_name,duration",
                "-of",
                "json",
                str(path),
            ],
    ...
    video_profile = str(video_stream.get("profile") or "").strip().lower()
    if video_profile != "high":
        raise RuntimeError(
            "generated MiniMax H3 MP4 must use the browser-compatible H.264 "
            f"High profile, got {video_stream.get('profile')!r}"
        )
