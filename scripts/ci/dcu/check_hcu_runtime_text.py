...
EXEMPT_PATH_PREFIXES = (
    "scripts/ci/dcu/tests/",
)
HCU_CODE_MARKER_RE = re.compile(
    r"\b(?:is_hcu|_is_hcu|is_dcu|_is_dcu|register_hcu_ci|register_dcu_ci)\b|"
...
def is_true_on_hcu(node):
    ...
    if isinstance(node, ast.Call):
        return function_name(node.func) in {"is_hcu", "is_dcu", "is_hip"}
    return False
...
def main(argv):
    ...
    if errors:
        ...
        print(" - HCU/DCU paths, HCU-specific files, and files with HCU code markers are covered.")
