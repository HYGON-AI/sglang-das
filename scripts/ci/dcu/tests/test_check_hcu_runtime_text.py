...
class HcuRuntimeTextCheckTest(unittest.TestCase):
    ...
    def test_hcu_and_dcu_paths_and_files_are_owned(self):
        self.assertTrue(CHECKER.is_hcu_owned_path("scripts/ci/dcu/example.sh"))
        self.assertTrue(CHECKER.is_hcu_owned_path("scripts/ci/hcu/example.sh"))
        self.assertTrue(CHECKER.is_hcu_owned_path("test/registered/dcu/test_x.py"))
        self.assertTrue(CHECKER.is_hcu_owned_path("test/registered/hcu/test_x.py"))
        self.assertTrue(
            CHECKER.is_hcu_owned_path(".github/workflows/pr-test-hcu.yml")
        )
        self.assertTrue(
            CHECKER.is_hcu_owned_path("python/sglang/test/dcu_utils.py")
        )
        self.assertTrue(CHECKER.is_hcu_owned_path("requirements_dcu.txt"))
        self.assertTrue(CHECKER.has_hcu_code_marker("if is_dcu():"))
        self.assertTrue(CHECKER.has_hcu_code_marker("HWBackend.DCU"))
    ...
    def test_dcu_compatibility_branch_violation_is_reported(self):
        source = '_is_dcu = True\nif _is_dcu:\n    print("AMD device")\n'
        errors = CHECKER.check_python(Path("sample.py"), source)
        self.assertEqual(len(errors), 1)
        self.assertIn("AMD device", errors[0].text)
    ...
    def test_main_returns_violation_and_tool_error_codes(self):
        with tempfile.TemporaryDirectory() as tmp:
            root = Path(tmp)
            bad = self.write_file(root, "test/registered/dcu/bad.py", 'print("AMD")\n')
            syntax = self.write_file(root, "test/registered/dcu/syntax.py", "if:\n")
